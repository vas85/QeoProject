...
//#endif
	...
	QeoMessagingHelper* messagingHelper = new QeoMessagingHelper();
	...
	QObject::connect(messagingHelper, SIGNAL(qeoMsgReceived(QString, QString)), mQeoJS, SLOT(onQeoMsgReceived(QString, QString)), Qt::QueuedConnection);

    app.exec();

	messagingHelper->StopListening();
}
